...
class CaseStudentProfileUpsert(BaseModel):
    ...
    allergy_visible: bool = Field(default=True, description="过敏史是否对非校长角色可见")
    underlying_conditions_visible: bool = Field(default=True, description="隐性疾病是否对非校长角色可见")
    other_health_notes_visible: bool = Field(default=True, description="其他健康信息是否对非校长角色可见")
