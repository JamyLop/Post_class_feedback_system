...
from app.models.user import ROLE_DEYU_DIRECTOR, ROLE_PARENT, ROLE_STUDENT, ROLE_TEACHER, User
...
def wx_bind(body: WxBindRequest, db: Session = Depends(get_db)):
    ...
    if body.invite_code:
        ...
        if body.role not in (ROLE_TEACHER, ROLE_DEYU_DIRECTOR, ROLE_STUDENT, ROLE_PARENT):
            raise HTTPException(status_code=400, detail="仅支持注册班主任、德育主任、学生或家长账号")
...
def register(body: RegisterRequest, db: Session = Depends(get_db)):
    """公开注册：班主任、德育主任、学生或家长必须使用对应角色的邀请码。"""
    if body.role not in (ROLE_TEACHER, ROLE_DEYU_DIRECTOR, ROLE_STUDENT, ROLE_PARENT):
        raise HTTPException(status_code=400, detail="仅支持注册班主任、德育主任、学生或家长账号")
    if db.query(User).filter(User.username == body.username).first():
        raise HTTPException(status_code=409, detail="用户名已存在")
