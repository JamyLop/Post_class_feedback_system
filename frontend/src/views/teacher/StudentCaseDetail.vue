    ...
    <template v-else-if="detail">
      ...
      <el-tabs v-model="active" class="case-tabs">
          ...
          <div v-if="subjectOptions.length" class="subject-workspace">
            ...
            <main class="subject-detail">
              ...
              <section class="subject-section">
                ...
                <div v-if="selectedPlan?.teacher_name" class="teacher-info-bar"><span class="teacher-label">授课老师</span><span class="teacher-name">{{ selectedPlan.teacher_name }}</span></div>
        ...
        <el-tab-pane label="生涯学案" name="versions">
          <div class="weekly-section">
            <div class="weekly-header">
              <div><strong>生涯学案</strong><span>当前 V{{ detail.version }} · 已存 {{ versions.length }} 个快照</span></div>
...
const stateDescription = computed(() => {
  ...
  return statusCopy[s]?.desc || '所有后续调整都会保留生涯学案，家长始终看到有版本依据的正式内容。'
})
...
const reviewRoleTip = computed(() => {
  if (auth.role === 'admin') return '管理员提交校级/校长督察'
  return ''
})
...
async function loadVersions() {
  ...
  try {
    ...
    ElMessage.error(e?.response?.data?.detail || '生涯学案读取失败')
    versions.value = []
  } finally {
...
async function handleArchive() {
  const { value } = await ElMessageBox.prompt('请输入归档原因（将保留生涯学案）', '归档确认', { confirmButtonText: '确认归档', cancelButtonText: '取消', inputPlaceholder: '例如：本周期已结束，归档留存' })
...
<style scoped>
.case-page { min-height: 100%; --radius-lg: 6px; --radius-md: 6px; --radius: 6px; }.page-skeleton { padding-top: 42px; }
.back-link { display: inline-flex; align-items: center; gap: 6px; margin: 2px 0 18px; padding: 6px 10px 6px 8px; color: var(--ink-secondary); background: var(--surface); border: 1px solid var(--line); border-radius: 999px; cursor: pointer; font-size: 13px; box-shadow: none; transition: border-color .18s, color .18s, background .18s; }.back-link:hover { color: var(--brand-strong); border-color: var(--line-strong); background: var(--surface); }
.case-header { display: grid; grid-template-columns: minmax(0, 1fr) auto; gap: 20px 24px; align-items: start; padding: 22px 24px 20px; background: var(--surface); border: 1px solid var(--line); border-radius: var(--radius-lg) var(--radius-lg) 0 0; box-shadow: none; }.case-heading { min-width: 0; }.title-line { display: flex; align-items: baseline; gap: 10px; flex-wrap: wrap; }.title-line h1 { margin: 0; color: var(--ink); font-size: 24px; font-weight: 600; line-height: 1.4; letter-spacing: normal; text-wrap: balance; }.title-suffix { color: var(--ink-muted); font-size: 14px; font-weight: 400; letter-spacing: normal; }
.case-meta { display: flex; flex-wrap: wrap; align-items: center; gap: 10px 14px; margin-top: 12px; color: var(--ink-muted); font-size: 12.5px; }.status-badge { display: inline-flex; align-items: center; gap: 7px; padding: 5px 10px; color: #8a5611; background: var(--warning-soft); border: 1px solid color-mix(in oklch, var(--warning) 14%, transparent); border-radius: 4px; font-weight: 400; font-size: 12px; letter-spacing: normal; }.status-badge.is-executing, .status-badge.is-adjusted, .status-badge.is-archived { color: #1a6b44; background: oklch(0.96 0.03 155); border-color: color-mix(in oklch, var(--success) 14%, transparent); }.status-dot { width: 7px; height: 7px; border-radius: 50%; background: currentColor; box-shadow: none; }
...
.state-banner { display: flex; align-items: flex-start; gap: 14px; margin: 0; padding: 14px 24px 16px; color: #7a4d12; background: var(--warning-soft); border: 1px solid color-mix(in oklch, var(--warning) 10%, var(--line)); border-top: 0; border-radius: 0 0 var(--radius-lg) var(--radius-lg); box-shadow: none; }.state-banner:not(.is-draft) { color: #1f5d3e; background: #f1f8f4; border-color: color-mix(in oklch, var(--success) 10%, var(--line)); }.state-icon { margin-top: 2px; font-size: 18px; flex-shrink: 0; }.state-banner strong { font-size: 14px; font-weight: 600; letter-spacing: normal; }.state-banner p { margin: 4px 0 0; max-width: 78ch; color: color-mix(in oklch, currentColor 74%, var(--ink)); font-size: 14px; line-height: 1.6; }
.transition-error { display: flex; align-items: center; gap: 10px; margin: 14px 0 0; padding: 12px 16px; color: #8a1f2a; background: #fef2f2; border: 1px solid #fecdd3; border-radius: 12px; font-size: 13px; box-shadow: none; }
.case-tabs { margin-top: 22px; }.case-tabs :deep(.el-tabs__header) { position: sticky; top: 0; z-index: 5; margin: 0; padding: 0 6px; background: var(--surface); border: 1px solid var(--line); border-radius: var(--radius-lg); box-shadow: none; }.case-tabs :deep(.el-tabs__nav-wrap) { padding: 4px; }.case-tabs :deep(.el-tabs__nav-wrap::after) { display: none; }.case-tabs :deep(.el-tabs__item) { height: 40px; padding: 0 16px; margin: 2px 4px 2px 0; color: var(--ink-secondary); font-size: 14px; font-weight: 400; border-radius: 4px; transition: background .18s, color .18s; }.case-tabs :deep(.el-tabs__item:hover) { color: var(--ink); background: var(--surface-soft); }.case-tabs :deep(.el-tabs__item.is-active) { color: var(--brand-strong); background: var(--brand-soft); font-weight: 600; box-shadow: none; }.case-tabs :deep(.el-tabs__active-bar) { display: none; }.case-tabs :deep(.el-tabs__content) { overflow: visible; padding-top: 20px; }
.profile-card { overflow: hidden; background: var(--surface); border: 1px solid var(--line); border-radius: var(--radius-lg); box-shadow: none; }.profile-card-header { display: flex; justify-content: space-between; align-items: flex-start; gap: 20px; padding: 22px 24px; border-bottom: 1px solid var(--line); background: var(--surface); }.profile-kicker { display: block; margin-bottom: 5px; color: var(--brand-strong); font-size: 11px; font-weight: 600; letter-spacing: .12em; }.profile-card-header h2 { margin: 0; font-size: 20px; letter-spacing: normal; }.profile-card-header p { margin: 7px 0 0; color: var(--ink-muted); font-size: 12.5px; }.profile-actions { display: flex; gap: 8px; flex-shrink: 0; }.profile-section { padding: 22px 24px 24px; }.profile-section + .profile-section { border-top: 1px solid var(--line); }.profile-section-title { display: flex; align-items: center; gap: 12px; margin-bottom: 16px; }.profile-section-title > div { display: grid; gap: 3px; }.profile-section-title h3 { margin: 0; font-size: 15px; }.profile-section-title p { margin: 0; color: var(--ink-muted); font-size: 11.5px; }.profile-grid { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 1px; margin: 0; overflow: hidden; background: var(--line); border: 1px solid var(--line); border-radius: 12px; }.profile-grid > div { min-width: 0; padding: 14px 16px; background: var(--surface-soft); }.profile-grid .profile-wide { grid-column: 1 / -1; }.profile-grid dt { margin-bottom: 6px; color: var(--ink-muted); font-size: 11.5px; font-weight: 600; }.profile-grid dd { margin: 0; color: var(--ink); font-size: 13.5px; line-height: 1.65; white-space: pre-wrap; overflow-wrap: anywhere; }.profile-copy-grid > div, .health-grid > div { min-height: 96px; }.health-section { background: color-mix(in oklch, var(--surface-soft) 58%, white); }.profile-form { padding: 0; }.profile-form-block { padding: 22px 24px 10px; }.profile-form-block + .profile-form-block { border-top: 1px solid var(--line); }.profile-form-block h3 { margin: 0 0 16px; font-size: 15px; }.profile-form-grid { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 0 16px; }.profile-form-wide { grid-column: 1 / -1; }.profile-form-copy :deep(.el-form-item), .profile-form-health :deep(.el-form-item) { align-self: start; }.profile-form :deep(.el-form-item__label) { padding-bottom: 6px; color: var(--ink); font-size: 12.5px; font-weight: 600; }.profile-form :deep(.el-select) { width: 100%; }.profile-form :deep(.el-input__wrapper), .profile-form :deep(.el-textarea__inner) { background: var(--surface-soft); border-radius: 10px; }.profile-form :deep(.el-textarea__inner) { line-height: 1.65; }.health-notice { display: flex; align-items: flex-start; gap: 8px; margin-bottom: 16px; padding: 11px 13px; color: #7a4d12; background: var(--warning-soft); border: 1px solid color-mix(in oklch, var(--warning) 14%, var(--line)); border-radius: 10px; font-size: 12px; line-height: 1.55; }.health-notice .el-icon { margin-top: 2px; flex-shrink: 0; }
...
.merged-parent-divider span { font-size: 12.5px; font-weight: 600; color: var(--ink); white-space: nowrap; }
...
.overview-toolbar { display: flex; justify-content: space-between; align-items: center; gap: 12px; margin-bottom: 14px; padding: 13px 16px; background: var(--surface); border: 1px solid var(--line); border-radius: var(--radius-lg); box-shadow: none; }.overview-toolbar-tip { color: var(--ink-muted); font-size: 12px; line-height: 1.5; }.overview-editing-note { margin-bottom: 14px; border-radius: var(--radius-lg); border: 1px solid color-mix(in oklch, var(--brand) 10%, var(--line)); box-shadow: none; }.overview-edit-form { padding: 20px 22px 6px; background: var(--surface); border: 1px solid var(--line); border-radius: var(--radius-lg); box-shadow: none; }.overview-edit-form :deep(.el-form-item) { margin-bottom: 14px; }.overview-edit-form :deep(.el-form-item__label) { padding-bottom: 6px; color: var(--ink); font-size: 13px; font-weight: 600; }.overview-edit-form :deep(.el-textarea__inner) { padding: 12px 14px; line-height: 1.7; background: var(--surface-soft); border-color: var(--line); }
...
.target-stage-label { color: var(--ink); font-size: 13.5px; font-weight: 600; line-height: 1.4; }
...
.target-row > span { color: var(--ink); font-size: 13.5px; font-weight: 600; }
...
.goal-subject { color: var(--ink); font-size: 12.5px; font-weight: 600; line-height: 1.2; text-align: right; letter-spacing: normal; }
...
.goal-score-copy strong { color: var(--ink); font-size: 14px; font-weight: 600; letter-spacing: normal; line-height: 1; }
.goal-score-copy span { color: var(--ink-muted); font-size: 10.5px; white-space: nowrap; }
.goal-score-copy em { margin-top: 3px; padding: 2px 6px; color: var(--warning); background: var(--warning-soft); border-radius: 5px; font-size: 10px; font-style: normal; font-weight: 600; line-height: 1.35; white-space: nowrap; }
...
.gaokao-timeline > header span { color: var(--brand-strong); font-size: 11px; font-weight: 600; }
...
.gaokao-month-head span { padding: 0 8px 7px; color: var(--ink-muted); font-size: 10.5px; font-weight: 600; text-align: right; }
.gaokao-month-head button { margin: 0 4px 5px; padding: 4px 6px; color: var(--ink-secondary); background: transparent; border: 0; border-radius: 5px; cursor: pointer; font: inherit; font-size: 10.5px; font-weight: 600; }
...
.gaokao-task-segment { z-index: 1; align-self: center; overflow: hidden; margin: 2px 4px; padding: 3px 6px; color: var(--ink); background: color-mix(in oklch, var(--line-strong) 70%, var(--surface)); border-radius: 4px; font-size: 10px; font-weight: 600; line-height: 1.3; text-overflow: ellipsis; white-space: nowrap; pointer-events: none; }
...
.stage-head strong { color: var(--ink); font-size: 13.5px; }.checkin-form-grid { display: grid; grid-template-columns: minmax(0, 1.6fr) minmax(160px, .9fr); gap: 14px; }.checkin-form :deep(.el-form-item) { margin-bottom: 12px; }.weekly-subjects { display: flex; flex-wrap: wrap; gap: 8px; margin-top: 10px; }.weekly-subject { display: inline-flex; align-items: center; gap: 8px; padding: 6px 12px; background: var(--surface-soft); border: 1px solid var(--line); border-radius: 999px; font-size: 12.5px; color: var(--ink-secondary); }.weekly-subject strong { color: var(--brand-strong); }.checkin-form :deep(.el-select) { width: 100%; }.checkin-form :deep(.el-form-item__label) { color: var(--ink); font-size: 12px; font-weight: 600; }.percent-suffix { margin-left: 6px; color: var(--ink-muted); font-weight: 600; }.checkin-list { display: grid; gap: 1px; background: var(--line); border: 1px solid var(--line); border-radius: 12px; overflow: hidden; }.checkin-row { display: grid; grid-template-columns: 72px minmax(0, 1fr); gap: 16px; padding: 14px 16px; background: var(--surface); }.checkin-row:first-child { border-radius: 12px 12px 0 0; }.checkin-row:last-child { border-bottom: 0; }.completion-rate { display: grid; align-content: center; justify-items: center; min-height: 54px; color: #fff; background: var(--brand-strong); border-radius: 10px; box-shadow: 0 2px 8px color-mix(in oklch, var(--brand) 22%, transparent); }.completion-rate strong { font-size: 16px; }.completion-rate span { margin-top: 1px; font-size: 10px; opacity: .9; }.checkin-row > div:last-child > strong { font-size: 13.5px; }.checkin-row p { margin: 4px 0; color: var(--ink-secondary); line-height: 1.6; font-size: 13px; white-space: pre-wrap; }.checkin-row time { color: var(--ink-muted); font-size: 11px; }.inline-empty { margin-top: 12px; padding: 16px; color: var(--ink-muted); background: var(--surface-soft); border: 1px dashed var(--line); border-radius: 10px; text-align: center; }.inline-empty p { margin: 0; font-size: 12.5px; }
.review-form-card { margin-bottom: 16px; padding: 20px 22px; background: var(--surface); border: 1px solid var(--line); border-radius: var(--radius-lg); box-shadow: none; }.review-form-header { display: flex; justify-content: space-between; align-items: baseline; gap: 12px; margin-bottom: 14px; padding-bottom: 12px; border-bottom: 1px solid var(--line); }.review-form-header strong { font-size: 14px; font-weight: 600; letter-spacing: normal; }.review-form-header span { color: var(--ink-muted); font-size: 11.5px; }.review-form-grid { display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); gap: 14px; }.review-form :deep(.el-form-item__label) { padding-bottom: 6px; color: var(--ink); font-size: 12px; font-weight: 600; }.review-form :deep(.el-select), .review-form :deep(.el-date-editor) { width: 100%; }.review-form :deep(.el-input__wrapper), .review-form :deep(.el-textarea__inner) { border-radius: 10px; }.review-form-actions { display: flex; justify-content: flex-end; }
.review-item-head { display: flex; gap: 8px; align-items: center; flex-wrap: wrap; margin-bottom: 6px; }.review-level { padding: 4px 8px; color: #fff; background: var(--brand); border-radius: 999px; font-size: 11px; font-weight: 600; letter-spacing: .02em; }.review-level.is-school { background: #7c3aed; }.review-level.is-principal { background: #be123c; }.review-level.is-head_teacher { background: var(--brand); }.review-level.is-subject { background: #0ea5e9; }.review-level.is-task_change { background: #d97706; }.review-decide-actions { display: flex; gap: 8px; margin-top: 10px; }.change-req-task { margin: 0 0 6px; font-size: 14px; }.dialog-tip { margin: 0 0 12px; font-size: 12.5px; color: #64748b; }.review-subject, .review-due { padding: 4px 8px; color: var(--ink-secondary); background: var(--surface-soft); border: 1px solid var(--line); border-radius: 999px; font-size: 11px; font-weight: 600; }.review-problem { margin: 0; color: var(--ink); font-size: 13.5px; line-height: 1.7; white-space: pre-wrap; }.review-action, .review-recheck { margin: 6px 0 0; color: var(--ink-secondary); font-size: 12.5px; line-height: 1.6; white-space: pre-wrap; background: var(--surface-soft); border: 1px solid var(--line); padding: 8px 10px; border-radius: 10px; }
.review-timeline { padding: 16px 20px; background: var(--surface); border: 1px solid var(--line); border-radius: var(--radius-lg); box-shadow: none; }.review-timeline :deep(.el-timeline-item__node) { border-color: var(--brand); background: var(--brand-soft); }.review-timeline :deep(.el-timeline-item__timestamp) { font-size: 11.5px; }
.weekly-section { background: var(--surface); border: 1px solid var(--line); border-radius: var(--radius-lg); box-shadow: none; padding: 18px 20px; }
