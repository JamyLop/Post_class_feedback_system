<template>
  <el-container class="app-shell">
    <el-aside width="228px" class="aside">
      ...
      <div class="aside-foot">坚持节奏 · 稳定提升</div>
    </el-aside>

    <el-container class="main-container">
      <el-header class="header">
        <div class="header-left">
          <span class="role-title">学生学习台</span>
          <span class="context-tag">高三主线</span>
        </div>
        <div class="header-right">
          <div class="user-profile">
            <span class="avatar-badge">{{ auth.user?.name?.slice(0, 1) || '学' }}</span>
            <div class="user-meta">
              <strong class="user-name">{{ auth.user?.name || '同学' }}</strong>
              <small class="user-role-text">学生</small>
            </div>
          </div>
...
:deep(.el-menu-item) {
  height: 38px; margin: 1px 8px; padding: 0 12px !important;
  border-radius: 8px; color: #8a97ad; font-size: 13.5px;
}
:deep(.el-menu-item:hover) { color: #d6deeb; background: rgba(255,255,255,0.05); }
:deep(.el-menu-item.is-active) {
  color: #fff; background: rgba(255,255,255,0.08); font-weight: 600; position: relative;
}
:deep(.el-menu-item.is-active)::before {
  content: ''; position: absolute; left: 0; top: 8px; bottom: 8px;
  width: 2.5px; border-radius: 999px; background: #5b7cff;
}
:deep(.el-menu-item .el-icon) { font-size: 15px; margin-right: 9px; }
.aside-foot {
  padding: 11px 16px;
  border-top: 1px solid var(--side-line);
  font-size: 11px; color: #5a6782;
}
