  ...
  <view class="page">
    ...
    <view v-if="isScheduler" class="card">
      ...
      <view class="form">
        ...
        <view class="two"><view class="field"><text class="label">星期 <text class="required">*</text></text><picker :range="weekdays" range-key="label" :value="weekdayIndex" @change="onWeekdayChange"><view class="picker"><text>{{ weekdayLabel }}</text><text>⌄</text></view></picker></view><view class="field"><text class="label">节次 <text class="required">*</text></text><picker :range="periodOptions" :value="periodIndex" @change="onPeriodChange"><view class="picker"><view><text>第 {{ form.period }} 节</text><text v-if="periodLabel(form.period)" class="picker-time">{{ periodLabel(form.period) }}</text></view><text>⌄</text></view></picker></view></view>
...
const periodOptions = computed(() => periodTimes.value.length ? periodTimes.value.map(item => item.period) : Array.from({ length: 12 }, (_, index) => index + 1))
...
function periodLabel(period, entry) {
  const time = entry?.start_time ? entry : periodTimes.value.find(item => item.period === period)
  return time?.start_time && time?.end_time ? `${time.start_time}-${time.end_time}` : ''
}
...
async function loadOptions() {
  // 时间配置不可用时仍保留 1-12 节，避免旧服务端阻断课表读取。
  const timeTask = listPeriodTimes().catch(() => [])
  if (!isScheduler.value) { periodTimes.value = await timeTask; return }
  const [times, classes, headTeachers, subjectTeachers] = await Promise.all([timeTask, listClasses(), listUsers('teacher', ''), listUsers('subject_teacher', '')])
