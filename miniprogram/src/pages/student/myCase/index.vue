<template>
  <view class="page">
    ...
    <EmptyState v-else title="暂无可查看档案" desc="班主任尚未发布可查看版本" />
  </view>
</template>
...
.name { font-size: 30rpx; font-weight: 600; color: var(--mp-ink); }
.meta { font-size: 24rpx; color: var(--mp-muted); display: block; margin-top: 6rpx; }
.state-banner { margin-top: 12rpx; padding: 14rpx 16rpx; border-radius: 12rpx; background: #F7F8FA; }
.state-banner.is-executing { background: #F0FDF4; }
.state-title { font-size: 24rpx; font-weight: 600; color: var(--mp-ink); display: block; }
...
.section-body { font-size: 24rpx; color: var(--mp-body); line-height: 1.7; white-space: pre-wrap; }
...
.subject-tag {
  font-size: 18rpx; color: var(--mp-primary); background: var(--mp-soft);
  padding: 4rpx 10rpx; border-radius: 14rpx;
}
.task-title { font-size: 24rpx; font-weight: 600; color: var(--mp-ink); }
